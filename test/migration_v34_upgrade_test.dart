...
import 'package:studioflow/models/domain/agendamento.dart';
import 'package:studioflow/models/domain/whatsapp_fila.dart';
...
void main() {
  ...
  test('Upgrade V33 -> V34 preserva colunas legadas e injeta as novas, mantendo compatibilidade', () async {
    ...
    await db.execute('CREATE TABLE clientes (id TEXT PRIMARY KEY, nome TEXT NOT NULL, whatsapp TEXT NOT NULL)');
    await db.execute('CREATE TABLE profissionais (id TEXT PRIMARY KEY, nome TEXT NOT NULL, whatsapp TEXT NOT NULL, cargo TEXT NOT NULL)');
    await db.execute('CREATE TABLE servicos (id TEXT PRIMARY KEY, nome TEXT NOT NULL, categoria TEXT NOT NULL, preco REAL NOT NULL, duracao_minutos INTEGER NOT NULL)');

    await db.insert('clientes', {'id': 'cli_1', 'nome': 'Cliente', 'whatsapp': '11999999999'});
    await db.insert('profissionais', {'id': 'pro_1', 'nome': 'Pro', 'whatsapp': '11999999999', 'cargo': 'cabeleireiro'});
    await db.insert('servicos', {'id': 'ser_1', 'nome': 'Corte', 'categoria': 'cabelo', 'preco': 50, 'duracao_minutos': 30});
    ...
    final colsCreditos = await db.rawQuery('PRAGMA table_info(creditos_comerciais)');
    ...
    expect(colsFila.any((c) => c['name'] == 'comercio_id'), isTrue); // Permanece!
