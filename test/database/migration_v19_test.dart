...
void main() {
  ...
  test(
    'Deve aplicar migração V19 com sucesso criando as 4 tabelas isoladas por comercio_id',
    () async {
      ...
      final historyCols = await db.rawQuery(
        "PRAGMA table_info('academy_search_history')",
      );
      ...
      final cacheCols = await db.rawQuery(
        "PRAGMA table_info('academy_search_cache')",
      );
      ...
      final count = Sqflite.firstIntValue(
        await db.rawQuery('SELECT COUNT(*) FROM academy_search_cache'),
      );
      expect(count, 2);
    },
  );
