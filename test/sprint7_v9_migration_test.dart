...
void main() {
  ...
  test('migração v8 para v9 preserva cliente, sessão e configuração', () async {
    final path = join(Directory.systemTemp.path, 'studioflow_v7_to_v8.db');
    ...
    expect(comercio['tipo_estabelecimento'], 'salao');
    expect(comercio['cor_principal'], '#70569A');
    expect(comercio['tema_automatico'], 1);
    final tabelasV8 = await db.rawQuery(
      "SELECT name FROM sqlite_master WHERE type = 'table' "
      "AND name IN ('recuperacoes_senha', 'dispositivos_sincronizacao')",
    );
    expect(tabelasV8, hasLength(2));
    expect(
      await db.query(
        'backups_logicos',
        where: 'versao_origem = ?',
    ...
    expect(
      await db.rawQuery(
        "SELECT name FROM sqlite_master WHERE type='table' "
        "AND name IN ('conflitos_sincronizacao', 'alteracoes_remotas', "
