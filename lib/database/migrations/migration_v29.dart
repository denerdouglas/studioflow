...
abstract final class MigrationV29 {
  static Future<void> executar(Database db) async {
    ...
    final comerciosRows = await db.rawQuery(
      'SELECT DISTINCT comercio_id FROM cardapio_itens',
    );
      ...
      if (flagRow.isEmpty) {
        ...
        if (itens.isNotEmpty) {
          ...
          await db.insert('catalogos_loja', {
            ...
            'descricao':
                'Catálogo importado automaticamente do cardápio antigo',
          ...
          for (var item in itens) {
            ...
            final exists =
                Sqflite.firstIntValue(
                  await db.rawQuery(
                    'SELECT COUNT(*) FROM estoque WHERE id = ?',
                    [id],
                  ),
                ) ??
                0;
        ...
        await db.insert('configuracoes', {'chave': flagChave, 'valor': 'true'});
      }
    }
  }
}
