...
import 'migrations/migration_v36.dart';
import 'migrations/migration_v37.dart';
import 'migrations/migration_v38.dart';
import 'migrations/migration_v39.dart';
import 'migrations/migration_v40.dart';
import 'migrations/migration_v41.dart';
import 'migrations/migration_v42.dart';
import 'migrations/migration_v43.dart';
import 'migrations/migration_v44.dart';
import 'migrations/migration_v45.dart';
import 'migrations/migration_v46.dart';
import 'migrations/migration_v47.dart';
...
abstract final class DatabaseSchemaLatest {
  static Future<void> criar(Database db, int version) async {
    ...
    await MigrationV33.executar(db);
    await MigrationV34.executar(db);
    await MigrationV35.executar(db);
    await MigrationV36.executar(db);
    await MigrationV37.executar(db);
    await MigrationV38.executar(db);
    await MigrationV39.executar(db);
    await MigrationV40.executar(db);
    await MigrationV41.executar(db);
    await MigrationV42.executar(db);
    await MigrationV43.executar(db);
    await MigrationV44.executar(db);
    await MigrationV45.executar(db);
    await MigrationV46.executar(db);
    await MigrationV47.executar(db);
  }
  ...
  static Future<void> migrar(
    ...
    if (versaoAnterior < 33 && novaVersao >= 33) {
      await MigrationV33.executar(db);
    }
    if (versaoAnterior < 34 && novaVersao >= 34) {
      await MigrationV34.executar(db);
    }
    if (versaoAnterior < 35 && novaVersao >= 35) {
      await MigrationV35.executar(db);
    }
    if (versaoAnterior < 36 && novaVersao >= 36) {
      await MigrationV36.executar(db);
    }
    if (versaoAnterior < 37 && novaVersao >= 37) {
      await MigrationV37.executar(db);
    }
    if (versaoAnterior < 38 && novaVersao >= 38) {
      await MigrationV38.executar(db);
    }
    if (versaoAnterior < 39 && novaVersao >= 39) {
      await MigrationV39.executar(db);
    }
    if (versaoAnterior < 40 && novaVersao >= 40) {
      await MigrationV40.executar(db);
    }
    if (versaoAnterior < 41 && novaVersao >= 41) {
      await MigrationV41.executar(db);
    }
    if (versaoAnterior < 42 && novaVersao >= 42) {
      await MigrationV42.executar(db);
    }
    if (versaoAnterior < 43 && novaVersao >= 43) {
      await MigrationV43.executar(db);
    }
    if (versaoAnterior < 44 && novaVersao >= 44) {
      await MigrationV44.executar(db);
    }
    if (versaoAnterior < 45 && novaVersao >= 45) {
      await MigrationV45.executar(db);
    }
    if (versaoAnterior < 46 && novaVersao >= 46) {
      await MigrationV46.executar(db);
    }
    if (versaoAnterior < 47 && novaVersao >= 47) {
      await MigrationV47.executar(db);
    }
  }
}
